...
def get_TPI(params, bvec1, graphs):
    ...
    (S, T1, T2, beta, sigma, l_tilde, b_ellip, upsilon, chi_n_vec, A,
        alpha, delta, K_ss, L_ss, C_ss, b_splus1_ss, n_ss, maxiter, mindist,
        TPI_tol, xi, diff, hh_fsolve) = params
    ...
    while (iter_TPI < maxiter) and (dist >= mindist):
        ...
        if hh_fsolve:
            ind = np.arange(S)
            bn_args = (rpath, wpath, ind, S, T2, beta, sigma, l_tilde, b_ellip, upsilon, chi_n_vec,
                bvec1, TPI_tol, diff)
