...
def get_SS(args, graph=False):
    (KL_init, beta, sigma, chi_n_vec, l_tilde, b, upsilon, S, alpha, A,
        delta, tau_k, tau_l, tau_c, alpha_X, alpha_D) = args
    dist = 10
    mindist = 1e-08
    maxiter = 500
    ss_iter = 0
    ...
    while dist > mindist and ss_iter < maxiter:
        ...
        x = X / S
        c1_guess = 1.0
        c1_args = (r, w, x, beta, sigma, chi_n_vec, l_tilde, b, upsilon, S,
                   tau_k, tau_l)
        results_c1 = opt.root(hh.get_bSp1, c1_guess, args=(c1_args))
        c1 = results_c1.x
        cvec = hh.get_recurs_c(c1, r, beta, sigma, S, tau_k)
        nvec = hh.get_n_s(cvec, w, sigma, chi_n_vec, l_tilde, b,
                          upsilon, tau_l)
        bvec = hh.get_recurs_b(cvec, nvec, r, w, x, tau_k, tau_l)
        B = aggr.get_B(bvec[:-1])
        D = alpha_D * Y
        K_new = B - D
    ...
    X_ss = X
    rev_params = (alpha, A, delta, tau_k, tau_l, tau_c)
    R_ss = tax.get_revenue(K_ss, L_ss, r_ss, w_ss, n_ss, b_ss, rev_params)
    G_ss = R_ss - r_ss * D_ss - X_ss


    print('SS interest rate is = ', r_ss)
    print('SS Government spending/GDP is = ' , G_ss/Y_ss)
