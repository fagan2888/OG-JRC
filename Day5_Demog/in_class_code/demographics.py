...
def get_fert(totpers, min_yr, max_yr, graph=False):
    ...
    if graph:
        ...
        output_fldr = "images/Demographics"
...
def get_mort(totpers, min_yr, max_yr, graph=False):
    ...
    cur_path = os.path.split(os.path.abspath(__file__))[0]
    mort_data = pd.read_table('data/mort_rates2011.csv', sep=',',
                              thousands=',')
    age_year_all = mort_data['Age'] + 1
    mort_rates_all = \
        (((mort_data['Male Mort. Rate'] *
         mort_data['Num. Male Lives']) +
    ...
    for i in range(max_yr):
        mort_rates_sub[i * num_sub_bins:(i + 1) * num_sub_bins] = \
            (1 - ((1 - np.float64(mort_rates_mxyr[i])) **
             (1 / num_sub_bins)))
...
def get_imm_resid(totpers, min_yr, max_yr, graph=True):
    ...
    '''
    cur_path = os.path.split(os.path.abspath(__file__))[0]
    pop_data = pd.read_table('data/pop_data.csv', sep=',',
                             thousands=',')
    pop_data_samp = pop_data[(pop_data['Age'] >= min_yr - 1) &
                             (pop_data['Age'] <= max_yr - 1)]
    # age_year_all = pop_data_samp['Age'] + 1
...
def get_pop_objs(E, S, T, min_yr, max_yr, curr_year, GraphDiag=True):
    ...
    imm_rates_mat = \
        np.hstack((np.tile(np.reshape(imm_rates_orig[E:], (S, 1)),
                  (1, fixper)), np.tile(np.reshape(imm_rates_adj[E:],
                                        (S, 1)), (1, T + S - fixper))))
    ...
    if GraphDiag:
        ...
        if omegaSSmaxdif > 0.0003:
            print('POP. WARNING: Max. abs. dist. between original ' +
                  'SS pop. distribution and period-T pop. distn is ' +
                  'greater than 0.0003. It is ' + str(omegaSSmaxdif) +
                  '.')
        else:
            print('POP. SUCCESS: orig. SS pop. dist is very close to ' +
                  'period-T pop. distribution. The maximum absolute ' +
