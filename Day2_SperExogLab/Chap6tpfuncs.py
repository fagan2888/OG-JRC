...
def LfEulerSys(bvec, *args):
    ...
    cvec, c_cnstr = c6ssf.get_cvec(rpath, wpath, bvec2, nvec)
    b_err_params = (beta, sigma)
    b_err_vec = c6ssf.get_b_errors(b_err_params, rpath[1:], cvec,
                                   c_cnstr, EulDiff)
    return b_err_vec
...
def get_TPI(params, bvec1, graphs):
    ...
    while (iter_TPI < maxiter_TPI) and (dist_TPI >= mindist_TPI):
        ...
        rpath = c6ssf.get_r(r_params, Kpath_init, Lpath)
        wpath = c6ssf.get_w(w_params, Kpath_init, Lpath)
        cpath, bpath, EulErrPath = get_cbepath(cbe_params, rpath, wpath)
        Kpath_new = np.zeros(T + S - 2)
        Kpath_new[:T], Kpath_cnstr = c6ssf.get_K(bpath[:, :T])
    ...
    Kpath = Kpath_new
    Y_params = (A, alpha)
    Ypath = c6ssf.get_Y(Y_params, Kpath, Lpath)
    Cpath = np.zeros(T + S - 2)
    Cpath[:T - 1] = c6ssf.get_C(cpath[:, :T - 1])
