...
endogenous labor and a small open economy assuption from Chapter
20 of the OG textbook.
...
delta         = scalar in [0,1], model-period depreciation rate of
                capital
r_star_annual = annual world real interest rate
r_star        = world real interest rate per model period
...
hh_solve_SS   = boolean, =False if solve inner-loop household problem by
...
hh_solve_TPI  = boolean, =False if solve inner-loop household problem by
                choosing c_1 to set final period savings b_{S+1}=0.
                Otherwise, solve the household problem as multivariate
                root finder with 2S-1 unknowns and equations
...
cur_ss_args    = length 12 tuple, current args to be passed in to
                 get_SS()
...
if SS_solve:
    ...
    ss_args = (S, beta, sigma, l_tilde, b_ellip, upsilon, chi_n_vec,
               A, alpha, delta, r_star, SS_tol, SS_EulDiff, hh_fsolve_SS)
...
# Don't compute steady-state, get it from pickle
    ...
    if (not ss_vars_exst) or (not ss_args_exst):
        ...
        cur_ss_args = (S, beta, sigma, l_tilde, b_ellip, upsilon,
                       chi_n_vec, A, alpha, delta, r_star, SS_tol, SS_EulDiff,
                       hh_fsolve_SS)
...
if TPI_solve:
    ...
    K1, K1_cnstr = aggr.get_K_s(bvec1)
    ...
    if K1_cnstr:
        ...
        tpi_params = (S, T1, T2, beta, sigma, l_tilde, b_ellip, upsilon,
                      chi_n_vec, A, alpha, delta, r_star, K_s_ss, L_ss, C_ss,
                      b_splus1_ss, n_ss, maxiter_TPI, mindist_TPI, TPI_tol, xi,
                      TPI_EulDiff, hh_fsolve_TPI)
        tpi_output = tpi.get_TPI(tpi_params, bvec1, TPI_graphs)

        tpi_args = (S, T1, T2, beta, sigma, l_tilde, b_ellip, upsilon,
                    chi_n_vec, A, alpha, delta, r_star, K_s_ss, L_ss, C_ss,
                    b_splus1_ss, n_ss, maxiter_TPI, mindist_TPI, TPI_tol, xi,
                    TPI_EulDiff, hh_fsolve_TPI, bvec1)
